...
# Make it a perfect square based on the largest dimension to avoid Android aspect ratio clipping
$size = [Math]::Max($w, $h)

# Add 8% padding so the corners don't get cut by Android's circular status bar mask
...
for ($x = 0; $x -lt $bmp.Width; $x++) {
    for ($y = 0; $y -lt $bmp.Height; $y++) {
        ...
        # Use safe 0.8 threshold so we don't accidentally erase lighter parts of the logo
...
$bmp.Dispose()
$img.Dispose()
Write-Host "Icons successfully cropped perfectly and safely converted!"
